...
{
    auto const invalid_fault_type =
        static_cast<Fault::Type>(std::numeric_limits<std::underlying_type_t<Fault::Type>>::max());
...
{
    REQUIRE(
        fmt::format("{}", Fault::Type::invalid_address) == get_name(Fault::Type::invalid_address));
    REQUIRE(fmt::format("{}", Fault::Type::invalid_instruction)
        == get_name(Fault::Type::invalid_instruction));
...
{
    REQUIRE(
        fmt::format("{}", Fault{Fault::Type::invalid_address, 0x123}) == "invalid_address@123h");
    REQUIRE(fmt::format("{}", Fault{Fault::Type::invalid_instruction, 0x456})
        == "invalid_instruction@456h");
    REQUIRE(fmt::format("{}", Fault{Fault::Type::invalid_digit, 0x768}) == "invalid_digit@768h");
    REQUIRE(fmt::format("{}", Fault{Fault::Type::empty_stack, 0xABC}) == "empty_stack@ABCh");
    REQUIRE(fmt::format("{}", Fault{Fault::Type::full_stack, 0xDEF}) == "full_stack@DEFh");
}
