...
TEST_CASE("Fault types define names", "[machine][fault]")
{
    ...
    REQUIRE(
        get_name(Fault::Type::invalid_instruction) == "invalid_instruction");
...
{
    REQUIRE(fmt::format("{}", Fault::Type::invalid_address)
        == get_name(Fault::Type::invalid_address));
    REQUIRE(fmt::format("{}", Fault::Type::invalid_instruction)
        == get_name(Fault::Type::invalid_instruction));
    REQUIRE(fmt::format("{}", Fault::Type::invalid_digit)
        == get_name(Fault::Type::invalid_digit));
    REQUIRE(fmt::format("{}", Fault::Type::empty_stack)
        == get_name(Fault::Type::empty_stack));
    REQUIRE(fmt::format("{}", Fault::Type::full_stack)
        == get_name(Fault::Type::full_stack));
}
